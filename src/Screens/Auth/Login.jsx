...
const Login = ({ navigation }) => {
  ...
  const register_token = (token) => {
    console.log(token);
    console.log(`${proyectSoftware}register-token-kid`);
    axios.post(
      `${proyectSoftware}register-token-kid`,
      {
        token
      },
    ).then(res => {
      let c = res.data.data
      console.log('respuesta token: ',c.child_id)
      if(c.child_id){
        console.log('hay boy_id', c.child_id)
        setUserInfo(c.child_id)
        let saveChildren
        = c.child_id + ''
        save('idBoy', saveChildren);
